...
const productSlice = createSlice({
    ...
    reducers:{       
        ...
        break;
        case "store2":
            state.store2Items = [...state.store2Items,{name:newproduct.item.name,price:newproduct.item.price}]
        state.store2Bill=state.store2Bill.filter(item => item.id !== newproduct.item.name)
        state.store2Total=0
        state.store2Bill.map( item => state.store2Total+=item.amount)  
        // console.log(state.store2Bill[0])
        break;
        case "store3":
            state.store3Items = [...state.store3Items,{name:newproduct.item.name,price:newproduct.item.price}]
        state.store3Bill=state.store3Bill.filter(item => item.id !== newproduct.item.name)
        state.store3Total=0
        ...
        default:alert("Not Deleted")
         ...
         addItems(state, action){
            ...
            case "store1":
             ...
             if(newproduct.newItem.quantity>0)   
